...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedCompletion_rendersSuccessfullyEmptyLoadedFeedAfterNonEmptyFeed() {
    ...
    func test_feedImageView_cancelImageURLRequestWhenInvisible() {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected image0 URL has been requested")
        
        sut.simulateFeedImageViewNotVisible(at: 1)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected image0 and image1 requests has been canceled")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, true, "Expected loading indicator for first view once user initiates a retry action for the first view")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view once user initiates a retry action for the first view")
        
        loader.completeImageLoading(at: 2)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for first view once first image loading completes successfully")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view once first image loading completes successfully")
    }
    ...
    func test_feedViewImage_doesNotRenderLoadedImageWhenNotVisibleAnymore() {
        ...
        XCTAssertNil(view?.renderedImageData, "Expected no rendered image when an image load finishes after the view is not visibile anymore")
    }
